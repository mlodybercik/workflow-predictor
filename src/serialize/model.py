...
class ModelSerializer:
    ...
    def __init__(self, model_path: Path, mode: str):
        self.model_path = model_path.absolute()
    ...
    def __enter__(self):
        ...
        if self.mode == "r":
            ...
            self._zip = ZipFile(self._buffer, "r", ZIP_DEFLATED)
        elif self.mode == "w":
            self._zip = ZipFile(self._buffer, "w", ZIP_DEFLATED)
        return self
    ...
    def save_model_to_zip(self, model: tf.keras.Model, stats: Optional[str] = None, *args, **params):
        if not self._zip:
    ...
    def load_model_from_zip(self) -> Tuple[tf.keras.Model, Dict[str, Any]]:
